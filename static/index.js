...
container.innerHTML = products.map(product => <div class="product-card bg-white group shadow-sm reveal-item rounded-2xl opacity-0 translate-y-8 scale-95 transition-all"> <a href="/product?id=${product.id}" class="block"> <div class="relative overflow-hidden bg-gray-400 aspect-square mb-4 rounded-2xl"> <img src="${product.image || '/static/img/placeholder.webp'}"  alt="${product.name}"  class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"> </div> <div class="p-4"> <p class="text-sm text-gray-500 mb-1" data-bn="মারকাযুস সুন্নাহ">Markazus Sunnah</p> <h3 class="heading-font text-xl font-semibold mb-2" data-bn="${product.name_bn || product.name}">${product.name}</h3> <p class="text-lg font-semibold" data-bn="৳${product.price}">${product.price} taka</p> </div> </a> </div>).join('');
...
gsap.set(".bg-layer", { scale: 1.2, autoAlpha: 0 });
...
sceneTl
    .to(".bg-layer", { scale: 1, autoAlpha: 0.8, duration: 2 })
    .to(".hero-title", { xPercent: 0, autoAlpha: 1, duration: 2 }, "-=1.5")
    .to(".item-left", { xPercent: 0, y: 0, rotation: 0, autoAlpha: 1, duration: 2, ease: "power2.out" }, "-=1.6")
    .to(".item-right", { xPercent: 0, y: 0, rotation: 0, autoAlpha: 1, duration: 2, ease: "power2.out" }, "-=1.6");
