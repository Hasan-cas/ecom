...
function initEventListeners() {
    ...
    // Collection tag checkboxes — Raw Materials is mutually exclusive with Gypsum Art / Jar Candle
    document.querySelectorAll('.collection-cb').forEach(cb => {
        cb.addEventListener('change', (e) => {
            const changed = e.currentTarget;
            if (changed.checked) {
                const allCbs = document.querySelectorAll('.collection-cb');
                if (changed.value === 'raw_materials') {
                    allCbs.forEach(other => { if (other.value !== 'raw_materials') other.checked = false; });
                } else {
                    allCbs.forEach(other => { if (other.value === 'raw_materials') other.checked = false; });
                }
            }
...
function renderDynamicFields() {
    const checked = document.querySelectorAll('.collection-cb:checked');
    const activeTags = Array.from(checked).map(cb => cb.value);
    ...
    activeTags.forEach(tag => {
        (COLLECTION_FIELDS[tag] || []).forEach(f => {
            if (!requiredFields.includes(f)) requiredFields.push(f);
        });
    });

    container.classList.remove('hidden');
    target.innerHTML = requiredFields.map(field => {
        const label = field.replace(/_/g, ' ').toUpperCase();
        return `
            <div>
                <label class="block text-[9px] uppercase font-bold text-navy/70 mb-1 tracking-widest">${label}</label>
                <input type="text" data-axis="${field}" class="variant-axis-input input-field !mt-0 !text-xs" placeholder="e.g. Value1, Value2">
            </div>
        `;
    }).join('');
